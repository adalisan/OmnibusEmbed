...
dirichlet_simulation_jofc_tradeoff_Kcond <- function(p, r, q, c.val,K,
		d           = p-1,
	...
	for(mc in 1:nmc) {
		...
		set.seed(mc)
		print(mc)
		mc.run<-try(run.mc.replicate.Kcond("dirichlet",p, r, q, c.val,K,
						d           = d,
...
dirichlet_simulation_jofc_tradeoff_par_Kcond <- function(p, r, q, c.val,K,
		d           = p-1,
	...
	par.mc.result<- foreach(mc =1:nmc,.packages=c("MASS","MCMCpack","smacof") ) %dopar% {
		...
		tmp<- run.mc.replicate.Kcond("dirichlet",p, r, q, c.val, K,  ##try(
				d           = d,
				p.prime.cond = p+q,
				Wchoice     = Wchoice, 
				pre.scaling = pre.scaling,
