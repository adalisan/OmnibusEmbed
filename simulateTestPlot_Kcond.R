simulate.generate.test.model.plot.Kcond<-function(model,params,run.parallel,K){
	wstar.List<-c()
	wstar.estim<-0
	sim.result<-with(params,{
				if ((model=="MVN") && run.parallel)  call.func<-gaussian_simulation_jofc_tradeoff_sf_Kcond
				if ((model=="MVN") && !run.parallel)  call.func<-gaussian_simulation_jofc_tradeoff_Kcond
				if ((model=="Dirichlet") && run.parallel)  call.func<-dirichlet_simulation_jofc_tradeoff_sf_Kcond
				if ((model=="Dirichlet") && !run.parallel)  call.func<-dirichlet_simulation_jofc_tradeoff_Kcond
				sim.res<-list()
				print("c")
				print(c.val)
				print(p)
				...
				args.for.func.call<-list(p=p, r=r, q=q, c.val=c.val,d=d,K=K,
						Wchoice     = "avg", 
						pre.scaling = TRUE,
						oos         = oos,
						alpha       = NULL,
...
#
							...
							if (add.plot.title) title(plot.title)
							par(lty=1)
							fname<- file.path('graphs',paste(c(model,"-FC-Tradeoff-",ifelse(oos,"OOS","noOOS"),"c",c.val," ",K,"-cond"),collapse=""))
						...
						#
							...
							#print(sim.res$FC.ratios$f.c)
							#print("wtFC.ratio.n  for MC replicates")
							#print(sim.res$FC.ratios$wtf.c)
							#
...
#
				...
				#temp
						...
						#wstar.List<-c(wstar.List,wstar.estim)
							...
							title(paste(model," model: power vs w plot"," ",K,"two condions"))
							fname<-file.path('graphs',paste(ifelse(oos,"OOS","noOOS"),model,"-power-w-c",c.val," ",K,"-cond",sep="", collapse=""))
