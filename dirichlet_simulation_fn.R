dirichlet_simulation <- function(p, r, q, c.val,
		d           = p-1,
		pprime1     = p+q+2,   # cca arguments
		pprime2     = p+q+2,   # cca arguments
		Wchoice     = "avg", 
		pre.scaling = TRUE,
		oos         = TRUE,
		alpha       = NULL,
	...
	## alpha: uniform on Delta^p (NULL) or given
	...
	initial.vectors.x1 <- matrix(0,data.dim,data.dim)
	initial.vectors.x2 <- matrix(0,data.dim,data.dim)
	eigenvalue.sample.x1 <- matrix (NA,data.dim,nmc)
	eigenvalue.sample.x2 <- matrix (NA,data.dim,nmc)
	...
	for(mc in 1:nmc) {
		...
		if (eigen.spectrum){
			...
			print(e.decomp.x$vectors)
			print(dim(eigenvalue.sample.x1))
			if (mc==1){
				initial.vectors.x1 <-	e.decomp.x$vectors
				initial.vectors.x2 <-	e.decomp.y$vectors
				eigenvalue.sample.x1[,1] <- t(e.decomp.x$values)
				eigenvalue.sample.x2[,1] <- t(e.decomp.y$values)
			}
			else{
				for (cur.evector.ind in 1:(data.dim)){
					cur.evector.match.1<-F
					cur.evector.match.2<-F
					for (init.evector.ind in 1:(data.dim)){
		...
		if (sim.grass){
			...
			grass.metric[mc, 2] <- grassmannian(Q1, Q2 * sa)           # scale after embedding
			geo.dist[mc, 2] <- geo_dist(Q1, Q2 * sa)
			Haursdorf [mc, 2] <- Haursdorf_dist (Q1, Q2 * sa)
			
			sb <- lm(as.vector(D1) ~ as.vector(D2) + 0)$coefficients   # scale before embedding
			grass.metric[mc, 3] <- grassmannian(Q1, Q2 * sb)           # scale before embedding
			geo.dist[mc, 3] <- geo_dist(Q1, Q2 * sb)
			Haursdorf [mc, 3] <- Haursdorf_dist (Q1, Q2 * sb)
			
			P <- polarity(X2t, X1t)
			grass.metric[mc, 4] <- grassmannian(Q1, Q2 %*% P * proc$s)     # scale after embedding & polarity
			geo.dist[mc, 4] <- geo_dist(Q1, Q2 %*% P * proc$s)
			Haursdorf [mc, 4] <- Haursdorf_dist (Q1,Q2 %*% P * proc$s)
			
			grass.metric[mc, 5] <- grassmannian(Q1, Q2 %*% proc$R * proc$s) # procrustes
			geo.dist[mc, 5] <- geo_dist(Q1, Q2 %*% proc$R * proc$s)
			Haursdorf [mc, 5] <- Haursdorf_dist (Q1,Q2 %*% proc$R * proc$s)
		}
	...
	plot.2 <- NULL
	if (eigen.spectrum){
		plot.1 <- plot.MC.evalues.with.CI(t(eigenvalue.sample.x1),"X1","blue",conf.int=TRUE,add=FALSE)
		plot.2 <- plot.MC.evalues.with.CI(t(eigenvalue.sample.x2),"X2","red",conf.int=TRUE,add=TRUE)
	}
	
	return(list(power=power,plot.1=plot.1,plot.2=plot.2,grassmannian=grass.metric,
					geodesic=geo.dist,	fc.pom = epsilon.pom,Haursdorf=Haursdorf))
}




dirichlet_simulation_jofc_tradeoff <- function(p, r, q, c.val,
		d           = p-1,
		pprime1     = p+q+2,   # cca arguments
		pprime2     = p+q+2,   # cca arguments
		Wchoice     = "avg", 
	...
	for(mc in 1:nmc) {
		...
		print(mc)
		mc.run<-try(
				run.mc.replicate("dirichlet",p, r, q, c.val,
						d           = d,
...
dirichlet_simulation_jofc_tradeoff_par <- function(p, r, q, c.val,
		d           = p-1,
	...
	par.mc.result<- foreach(mc =1:nmc,.packages=c("MASS","MCMCpack","smacof") ) %dopar% {
		...
		source("./src/oosIM.R")
		sink(file=paste("debug-D-",mc,collapse=""))
		set.seed(mc)
		...
		tmp<-run.mc.replicate("dirichlet",p, r, q, c.val,
						d           = d,
						pprime1     = pprime1,   # cca arguments
						pprime2     = pprime2,   # cca arguments
						Wchoice     = Wchoice, 
		...
		sink(file=paste("traceback-debug-D-",mc,".txt",collapse=""))
		traceback()
		sink()
...
#		means[i,]      <- par.mc.result[[(num.value.per.mc.rep*(i-1))+6]]
	...
	FC.ratios<-list(f.c=F.to.C.ratio,wtf.c=wtF.to.C.ratio)
	print(str(FC.ratios))
	return (list(power=power,power.cmp = power.cmp, conting.table=agg.cont.table,conting.table.list=cont.tables,
